...
OBS: The reason for the NAT adapter choice was not on purpose as you can also use an Bridge adapter to establish internet access, but basically there is a WAN connection and a LAN connection with also the chance to configure
the NAT internally and then the internal network hosts will access the internet.

## Installation Process

With the ISO inserted on the VM, the installation process starts.

You first select the language, as my ISO was download with the main language as English, the selected languague will be English.
...
There are two options for the Desktop environment: Just Command Line or the Graphical Interface version, here the selected one will be the Standard Evaluation and Desktop Experience.
...
You can then click to accept the terms and select the Custom Installation.
...
Just click Next to install the Operating System into our 50 GB Hard Disk.
...
guessable are not accepted, the one chosen here was "P@ssword1"
...
Our domain will need few things before the ad installation process start, we are going to identify the network cards and then select a hostname for our server, to identify the network cards we can do with cmd by typing [ipconfig] to show both the network adapters.
...
To make it easy, remember that the NAT reserved IP for Virtual Box is commonly with the start "10.0.2." but some routers identify with the connection-specific DNS as it's own hostname, in my case "dlinkrouter" so this Ether 2 is my NAT card. Not only this can help but also as a standard configuration the Windows give one Automatic Private IP Address(APIPA) to each interface that doesn't receive an IP from some DHCP server, just for guarantee the LAN communication, so the Ether is our internal network.

To rename then, go to Network Connections, click with the right button over then and then click on "Rename".
...
Now we go to the Internal Network properties and change the IPv4 address, here we can choose the "192.168.0.1", a 256 hosts masks as "255.255.255.0", as we are the internal network gateway we don't need one and for the DNS address we can set the Loopback IP or "127.0.0.1" and then click OK.
...
After we rename our server, to make it easy to identify you can select any name as "DC", "DOMAINCONTROLLER","DOMAIN" etc.
...
After the machine restart we can now start the AD installation and configuration, click on the "Add roles and features", this options is used to add some features to our server such as the Active Directory, DNS, DHCP server, Remote Access and many others.
...
Now select the "Active Directory Domain Services" and "DHCP Server", it will appear an window to confirm the features, just click "Add Features".
...
After it you can just click Next in the next windows as they are just showing some advisories, but you can trust and then just click Next and then Install.
...
As we don't have a DNS delegated we click Nex in the DNS Options.
...
The Review will show you the options chosen but also the option "View Script" will show you the powershell script that can be used to replicate this configuration in others domains, if you want to you can save anywhere as a ".ps1" file, then click Next.
...
After the installation the server will reboot again but now, you can see that the LogOn screen show not only the username but also our domain, the standard pattern for the usernames show in the lock screen is: DOMAIN\User
...
As we have our AD Domain configured and operating, we are going now to configure the DHCP server, for this go the Server Manager window and click in "Tools" and then click on "DHCP Server".

Click on the Domain shown and then right click on the IPv4 and select "New Scope", click Next.
...
Click Next and then select the Scope Name, here we can name the server as the range like this, you can call as you want but if you want to manage others scopes make it easy to remember, click Next.
...
The lease duration it from you, again in a enterprise environment, some places such as a Coffee Shop don't need an 8 days of lease time as the IP will be reserved to the specific host that claimed until the DHCP server make other offer after the lease time passes, click on Next.
...
The Router or Default Gateway is where we configure our internal IP server address in our case will be "192.168.0.1", after type it click on Add and then Next.
...
For the DNS server make sure the Parent Domain are the one you delegate as the root forest and the IP address does match with the server internal IP, click Next.
...
Then click in the option "Yes, I want to activate this scope now", click Next and close.
...
We have the DHCP server but we need to authorize it, to do this right click our domain under DHCP and click "Authorize", to make sure is active click in the IPv4 and in the right window will appear our scope.
...
To create a user search for "Active Directory Users and Computers" and then click on it, click on our domain and then will be listed our Organizational Units, they work as directories, you can create one just for the users by right click in the domain and then click in "New" and "Organizational Unit" and then call it as "_Users" or create a new user under the already created User directory, click on it and then "New" and "User".
...
It will open the new user creation window, insert the First, Last names and then the user logon. An commonly standards of users in some environments are: first name letter + last name all in lowercase, so for the John Wick user will have the "jwick" user logon name, click Next.
...
Select the password and again the password policy takes the lead to make sure you follow every requirement, you can simulate an real environment by leaving selected the "User must change password at next logon" or just select the "Password never expires" option to leave the password as you configure.
...
For the user machine we can use the same specifications as the server but only changing the amount of network interface cards and the ISO used, as we are gonna need to install a desktop operational system such as Windows 10, 11 or use a preconfigured OVA image, the network interface card used is configured here to "Internal Network".
...
Now to insert the user in the domain we can search for the System options or just press [Windows+PauseBreak], again mine will appear as PT_br but the spot is the same for both languages, click in "Change Settings" or "Alterar Configurações" and then click in "Change" or "Alterar" and then you click on "Member of" ou "Membro de" and type the domain you configured, here the domain typed was the one we configure as "sentinels.com" and click OK.
...
Then it will appear a welcome screen and the machine will be rebooted.
...
Again, as to enter the Domain LogOn screen insert via the Virtual Keyboard the combination of keystrokes [Ctrl+Alt+Delete].
...
To check if you are really inside the domain, press [Windows+PauseBreak] or go to System options, it should appear the hostname and the domain you entered.
...
If you do want to also access internet with the users inside the domain, you can add the NAT feature to your server.
...
After the install, click in the "Tools" options and then in "Routing and Remote Access". Right click the domain hostname and then click in 
"Configure and Enable Routing and Remote Access", click in Next.
...
After it, click in Finish, it will configure.
...
The domain icon now will have an green arrow pointing up, this means that the NAT configuration was successfull.
...
One great reference to this project was the youtuber [Josh Madakor].
